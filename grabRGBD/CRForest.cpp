...
void CRForest::extractAllPatches(const cv::vector<cv::Mat*> &image, std::vector<CPatch> &patches) const{
    ...
    for(int j = 0; j < image.at(0)->cols - conf->p_width; j += conf->stride){
        for(int k = 0; k < image.at(0)->rows - conf->p_height; k += conf->stride){
            ...
            int classNum = -1;//classDatabase.search(dataSet.className);
            ...
            tPatch.setPatch(temp, image, tempDataset, classNum);
...
void CRForest::loadForest(){
    ...
    nameList.clear();
    for(int i = 0; i < classDatabase.vNode.size(); ++i){
        nameList.push_back(QString::fromStdString(classDatabase.vNode.at(i).name));
    }
    emit sendClassNames(nameList);
}

// name   : detect function
// input  : image and dataset
// output : classification result and detect picture
void CRForest::detection(cv::Mat* rgb,cv::Mat* depth){//,
    ...
    //        cToString << k;
    //        cv::imwrite(cToString.str() + ".png",*(features.at(k)));
    //    }
    ...
    extractAllPatches(features, patches);
    ...
    //    cv::namedWindconnect(settingD,SIGNAL(sendConfig(CConfig)),forest,SLOT(receiveConfig(CConfig)));
    //ow("test");
    //    cv::imshow("test",outputImageC1);
    //    cv::waitKey(0);
    //    cv::destroyWindow("test");

    std::cout << showClassNum << std::endl;
    ...
    if(showClassNum != -1){
        ...
        cv::imwrite("test.png",outputImageColorOnly.at(showClassNum));
    }
...
void CRForest::recieveWhatDialogWantToShow(QString showClass){
    std::cout << "hoi kita : " << showClass.toStdString() << std::endl;

    int classNum = classDatabase.search(showClass.toStdString());
    if(classNum == -1)
        std::cout << "error! request class is not found!" << std::endl;
