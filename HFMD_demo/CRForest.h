...
class CRForest : public QObject{
 ...
 public:
  ...
  void loadForest();
  void extractAllPatches(const cv::vector<cv::Mat*> &image,
...
public slots:
  void detection( cv::Mat* rgb, cv::Mat* depth);//, std::vector<double> &detectionResult, int &detectClass) const;
  void recieveWhatDialogWantToShow(QString);
  void receiveConfig(CConfig *);

signals:
  void detectedResult(std::vector<detectionResult*>);
...
inline void CRForest::minFilter(cv::Mat* src, cv::Mat* des, int fWind) const{
  ...
  cv::Mat desTemp(src->rows, src->cols, CV_8U), vTemp;
    ...
    }// For imagQ_DECLARE_METATYPE(MyClass)e height
...
Q_DECLARE_METATYPE(cv::vector<cv::Mat*>)
